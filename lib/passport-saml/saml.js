...
SAML.prototype.initialize = function (options) {
  ...
  if (!options.validateRedirectSignature) {
    options.validateRedirectSignature = true;
  }
...
SAML.prototype.validateRedirectResponse = function (container, callback) {
  var self = this;
  var uriDecoded = decodeURIComponent(container.SAMLResponse);
  var deflated = new Buffer(uriDecoded, 'base64');
  zlib.inflateRaw(deflated, function(err, buffer) {
    if (err) console.error(err);
    var xml = buffer.toString('ascii');
    self.validateResponseXml(xml, self.options.validateRedirectSignature, callback);
  });
};
...
SAML.prototype.validatePostResponse = function (container, callback) {
  ...
  self.validateResponseXml(xml, true, callback);
};

SAML.prototype.validateResponseXml = function (xml, shouldValidateSignature, callback) {
  var self = this;
  ...
  .then(function() {
    ...
    if (shouldValidateSignature && self.options.cert && self.validateSignature(xml, doc.documentElement, self.options.cert)) {
      validSignature = true;
    }
            ...
            if (status) {
              ...
              if (statusCode && statusCode[0].$.Value === "urn:oasis:names:tc:SAML:2.0:status:Responder") {
                ...
                if (nestedStatusCode && nestedStatusCode[0].$.Value === "urn:oasis:names:tc:SAML:2.0:status:NoPassive") {
                  if (shouldValidateSignature && self.options.cert && !validSignature) {
                    throw new Error('Invalid signature');
                  }
...
SAML.prototype.validatePostRequest = function (container, callback) {
  ...
  self.validateRequestXml(xml, callback);
};

SAML.prototype.validateRedirectRequest = function (container, callback) {
  var self = this;
  var uriDecoded = decodeURIComponent(container.SAMLRequest);
  var deflated = new Buffer(uriDecoded, 'base64');
  zlib.inflateRaw(deflated, function(err, buffer) {
    if (err) console.error(err);
    var xml = buffer.toString('ascii');
...
SAML.prototype.validateRequestXml = function (xml, callback) {
  var self = this;
  var parserConfig = {
    explicitRoot: true,
    tagNameProcessors: [xml2js.processors.stripPrefix]
